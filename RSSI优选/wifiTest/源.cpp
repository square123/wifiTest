...
private:	
	...
	struct syscProbe	//同步多探针的结构体	
	{
		...
		unsigned char selMumac[6];
		char Rssi;
	}syscStr[3][ProbeNum][sameTimeMacNum];//三维结构体数组，要寄存的时间、探针数量、每秒钟存储的数据
	...
	void AllreduceSyscProbe(syscProbe Allsysc[3][ProbeNum][sameTimeMacNum],syscProbe Allsysced[3][ProbeNum][sameTimeMacNum]);//将整个结构体数组缩减
...
Probe::Probe()
{
	storeIndex=-1;//要更新的数据存储索引，将该变量初始为-1，在后续存储时可以直接从索引0开始存储
	processIndex=1;//要使基准组的索引号，从2开始，必须先让其为1
...
void Probe::probeProcess()
{
	...
	{	
		if (syscTime<charToTimeInt(retime))//输出时间不同时，将标志位保存下
		{
			//测试显示部分
			
			probeSysc(0,fpSysc);//处理函数
			printf("要同步的时间池: ");
			...
			storeIndex=(storeIndex+1)%3;//存储索引加1
			processIndex=(processIndex+1)%3;//处理索引加1，因为函数是同步函数是运行存储之前，所以要将处理的索引滞后
...
{
	//初始化部分
	memset(syscResult,0,sizeof(syscProbed)*sameTimeMacNum);//用于多探针集合表格式
	syscProbe syscStrEd[3][ProbeNum][sameTimeMacNum];//用于存储紧凑后的变量
	...
	for (int m=0;m<sameTimeMacNum;m++)//该循环是将baseIndex探针中processIndex时间下的结构体数据与其他索引进行比较
	{
		...
		for(int n=0;n<ProbeNum;n++)//该循环是剔除baseIndex的索引，与方便与其他探针比较,n表示探针索引
		{
			if(n==baseIndex)//跳出基准索引
			{
				continue;
			}	
			for (int k=0;k<3;k++)//遍历时间
			{	
				bool skip=false;
				if (!(timeCompare(syscStrEd[k][n][0].Timestamp,syscStrEd[processIndex][baseIndex][0].Timestamp,1)))//当时间不合格时，直接跳出,因为一块的时间是相同的，所以用0就可以
				{
				...
				{	
					...
					if(memcmp(syscStrEd[processIndex][baseIndex][m].selMumac,syscStrEd[k][n][l].selMumac,sizeof(unsigned char)*6)==0)//如果匹配就把时间导入
					{
	...
	for (int q=0;q<jianshao;q++)
	{
		...
		if(outflag)
		{
			...
			for (int ii=0;ii<14;ii++) 
			{
				...
				printf("%c", syscResult[q].Timestamp[ii]);
			}
			fprintf(fpSysc,",");
			printf(" ");
			...
			printf("%02X:%02X:%02X:%02X:%02X:%02X ",syscResult[q].selMumac[0], syscResult[q].selMumac[1], syscResult[q].selMumac[2], \
				syscResult[q].selMumac[3], syscResult[q].selMumac[4], syscResult[q].selMumac[5]);
			for (int r=0;r<ProbeNum;r++)
			{
				fprintf(fpSysc,"%d", syscResult[q].Rssi[r]);
				printf("%d", syscResult[q].Rssi[r]);
...
//memset,memcpy.memcmp,bug总结，数组类的可以不取地址，非数组类，结构体必须要取地址