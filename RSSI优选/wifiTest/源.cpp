...
Probe::Probe()
{
	...
	for(int i=0;i<ProbeNum;i++)
	{
		...
		flag[i]=false;//标志位初始化
		saveFinshFlag[i]=false;//存储标志位初始化
...
void Probe::probeSysc(int baseIndex,FILE *f)//探针同步函数,将同步后的数据存储到一个新的表中 数据有丢失，是因为考虑的不够周全，并没有完全解决同步的问题，之前的一些确实是有bug
{
	...
	memset(syscStrEd,0,(sizeof(syscProbe)*3*ProbeNum*sameTimeMacNum));//清空
	...
	for (int m=0;m<sameTimeMacNum;m++)//该循环是将baseIndex探针中processIndex时间下的结构体数据与其他索引进行比较
	{
		...
		for(int n=0;n<ProbeNum;n++)//该循环是剔除baseIndex的索引，方便与其他探针比较,n表示探针索引
		{
			...
			for (int k=0;k<3;k++)//遍历时间,这里做了一点小改动，希望从最小的时间开始记录。就将k变成((processIndex+2+k)%3)，默认最先到的最好、、、、、！！！！需要验证
			{	
				bool skip=false;
				if (!(timeCompare(syscStrEd[((processIndex+2+k)%3)][n][0].Timestamp,syscStrEd[processIndex][baseIndex][0].Timestamp,1)))//当时间不合格时，直接跳出,因为一块的时间是相同的，所以用0就可以
				{
				...
				{	
					...
					if(memcmp(syscStrEd[processIndex][baseIndex][m].selMumac,syscStrEd[((processIndex+2+k)%3)][n][l].selMumac,sizeof(unsigned char)*6)==0)//如果匹配就把时间导入
					{
