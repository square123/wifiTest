...
void Probe::probeSysc(int baseIndex,FILE *f)//探针同步函数,将同步后的数据存储到一个新的表中 数据有丢失怎么解决？？？
{
	...
	syscProbe syscStrEd[3][ProbeNum][sameTimeMacNum];//定义用于存储紧凑后的变量
	memset(syscStrEd,0,(sizeof(syscProbe)*3*ProbeNum*sameTimeMacNum));//清空&初始化
	...
	for (int m=0;m<sameTimeMacNum;m++)//该循环是将baseIndex探针中processIndex时间下的结构体数据与其他索引进行比较
	{
		...
		for(int n=0;n<ProbeNum;n++)//该循环是剔除baseIndex的索引，方便与其他探针比较,n表示探针索引
		{
			...
			for (int k=0;k<3;k++)//遍历时间,这里做了一点小改动，希望从最早的时间开始记录。就将k变成((processIndex+2+k)%3)，默认最先到的最好、、、、、！！！！需要验证
			{	
				bool skip=false;
				...
				{	
					...
					if(memcmp(syscStrEd[processIndex][baseIndex][m].selMumac,syscStrEd[((processIndex+2+k)%3)][n][l].selMumac,sizeof(unsigned char)*6)==0)//如果匹配就把RSSI值导入
					{
