	...
	for (int m=0;m<sameTimeMacNum;m++)//该循环是将baseIndex探针中processIndex时间下的结构体数据与其他索引进行比较
	{
		...
		for(int n=0;n<ProbeNum;n++)//该循环是剔除baseIndex的索引，方便与其他探针比较,n表示探针索引
		{
			...
			{	
				...
				{	
					if (memcmp(&syscStrEd[((processIndex+2+k)%buffNum)][n][l],&zeroSysc,sizeof(syscProbe))==0)
					{
						break;//因为已经让数据紧凑了，所以可以直接使用break
					}
					if(memcmp(syscStrEd[processIndex][baseIndex][m].selMumac,syscStrEd[((processIndex+2+k)%buffNum)][n][l].selMumac,sizeof(unsigned char)*6)==0)//如果匹配就把RSSI值导入
					{
						syscResult[m].Rssi[n]=syscStrEd[((processIndex+2+k)%buffNum)][n][l].Rssi;
						skip=true;
						break;
					}
				}
