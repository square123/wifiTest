...
void overlapPro::bwMixToOrbit(vector<bwMix> &src)  //这里还可能存在数据突然检测不到的可能，在把论文投出去后，还需要进一步完善
{
	//先找出来有多少重复的标号
	std::set<int> S;//通过设置s筛选数据
	for (int i = 0; i < src.size(); i++)
	...
	//对重复标号的重新合并为一个矩阵，计算其重心,完后空域上的统一
...
void overlapPro::process(Mat &src)
{
	vector<bwMix> picBwMix;
	picBwMix.reserve(100);
	selectArea(src, selectedMat, 10, -1);//通过函数对一些连通域小的进行筛选  以后可以根据实地判断筛选 针对稀疏人物目标的跟踪
