...
public:
	...
	void bwMixToOrbit(vector<bwMix> &src);//负责输出轨迹
	void process(Mat &src,Mat &colorSrc, Mat &oriSrc);//将前景提取的方法独立出来，该类要完成的只是overlap跟踪
	void timeRawProcess();//负责将信息压缩成1秒
	void deHue(Mat &src,Mat &colorSrc,Mat &oriImgHue,Mat &dst);//用来剔除hue所占据的空间 
	Mat selectedMat;
	Mat hullMat;
	Mat showColor;
	vector<bwMix> prePicBwMix;//缓冲变量便于更新 缓冲变量应该从这里做文章
